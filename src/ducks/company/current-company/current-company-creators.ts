...
export const editCompany = createAsyncThunk<void, EditCompanyInterface, baseStoreType>(
...
export const editCompanyCoords = createAsyncThunk<void, { lat: number; long: number }, baseStoreType>(
  'currentCompany/editCompanyCoords',
  async ({ lat, long }, { dispatch, getState }) => {
    const { token } = getState().auth.tokens;
    const { currentCompany } = getState().company.currentCompany;
...
export const deleteCompany = createAsyncThunk<void, () => void, baseStoreType>(
  'currentCompany/deleteCompany',
